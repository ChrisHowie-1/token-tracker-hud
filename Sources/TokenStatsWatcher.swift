...
public class TokenStatsWatcher: ObservableObject {
    ...
    public func loadStats() {
        ...
        // 2. When running on MacBook, fetch live stream from Mac Studio LAN daemon first (sub-millisecond, zero iCloud latency)
        ...
        session.dataTask(with: request) { [weak self] data, response, error in
            ...
            // 3. Fallback to iCloud synced file if LAN is unreachable
            if fileManager.fileExists(atPath: self.icloudStatsPath),
    ...
    @discardableResult
    private func decodeData(_ data: Data) -> Bool {
        do {
